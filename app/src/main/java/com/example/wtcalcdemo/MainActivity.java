       ...
       // radGroupConv.getCheckedRadioButtonId() --> -1 if nothing is onContextItemSelected()
        ...
        btnConvertWt.setOnClickListener(new View.OnClickListener() {
            public void onClick(View v) {
                int selectedRadioBtnId = radGroupConv.getCheckedRadioButtonId();
                String wtTextInput = editTextInputWt.getText().toString();
                double wtTextInputDouble = Double.parseDouble(editTextInputWt.getText().toString());
                String formattedOutputWeight = "";

                if (selectedRadioBtnId == -1) {
                    Toast.makeText(MainActivity.this, "Please select a conversion type", Toast.LENGTH_SHORT).show();
                } else if (wtTextInput.isEmpty()) {
                    Toast.makeText(MainActivity.this, "Please enter a weight", Toast.LENGTH_SHORT)
                            .show();
                } else if (selectedRadioBtnId == R.id.radBtnLbsToKgs && wtTextInputDouble > 1000) {
                    Toast.makeText(MainActivity.this, "Input weight must be <= 1000", Toast.LENGTH_SHORT).show();
                } else if (selectedRadioBtnId == R.id.radBtnKgsToLbs && wtTextInputDouble > 500) {
                    Toast.makeText(MainActivity.this, "Input weight must be <= 500", Toast.LENGTH_SHORT).show();
                } else {
                    if (selectedRadioBtnId == R.id.radBtnLbsToKgs) {
                        double inputWeightInPounds = Double.parseDouble(wtTextInput);
                        double outputWeight = inputWeightInPounds / 2.2;
                        DecimalFormat df = new DecimalFormat("#.## Kgs");
                        formattedOutputWeight = df.format(outputWeight);
                    } else if (selectedRadioBtnId == R.id.radBtnKgsToLbs) {
                        double inputWeightInKilos = Double.parseDouble(wtTextInput);
                        double outputWeight = inputWeightInKilos * 2.2;
                        DecimalFormat df = new DecimalFormat("#.## Lbs");
                        formattedOutputWeight = df.format(outputWeight);
                    }
                }

                txtViewResults.setText(formattedOutputWeight);
            }
        });
    }
}